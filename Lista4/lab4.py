...
def print_best_results(best_results: List, best_func: List):
    ...
    print("0.A\n1.B\n2.C\n3.A+B-C\n4.A+(B-C)/2\n5.B+(A-C)/2\n6.(A+B)/2\n7.New standard")
    print("Best for:")
    print("file  :", best_results[3], "fun", best_func[3])
    print("red   :", best_results[0], "fun", best_func[0])
    print("green :", best_results[1], "fun", best_func[1])
    print("blue  :", best_results[2], "fun", best_func[2])
...
def main(file_name: str):
    ...
    if is_uncompressed and byte_collector.byte_array[16] == 0x20:
        ...
        raise ValueError('The image is compressed!')
    file_stats(image_stats)
    predictions(image_stats)

if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Incorrect number of arguments! Enter: python lab4.py filename.tga")
        exit(1)
    main(sys.argv[1])